...
const SidebarGroupedList = <T extends { id: string, start_date?: string, end_date?: string }>({
  ...
  return (
    <div 
      className={`side-todo-extension ${isVisible ? 'visible' : ''} ${isResizing ? 'resizing' : ''} ${className}`} 
      style={{ width: isVisible ? `${width}px` : '0' }}
    >
      <div className="side-extension-resizer" onMouseDown={onResizerMouseDown} />
      <div className="side-extension-header">
        <h3>{title}</h3>
        <button className="side-close-btn" onClick={onClose} aria-label="Collapse panel">
          <ChevronLeft size={20} />
        </button>
      </div>
              ...
              return (
                <div key={month} className={`side-month-group ${isExpanded ? 'expanded' : ''}`}>
                  ...
                  <div className="side-month-content">
                    {/* 1. 기간형 할 일 (최상단 고정) */}
                    {rangeItems.length > 0 && (
                      <div className="side-date-group range-group">
                        <div className="side-date-badge range-badge">
                          <Calendar size={12} />
                          <span>기간형 일정</span>
                        </div>
                        <ul className="side-todo-list">
                          {rangeItems.map((item) => (
                            <div key={item.id} className="side-range-item-wrapper">
                              <div className="side-range-duration">
