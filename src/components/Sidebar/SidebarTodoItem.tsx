...
const SidebarTodoItem: React.FC<SidebarTodoItemProps> = ({ 
  ...
  if (isEditing) {
    return (
      <li 
        className="side-todo-item editing"
          ...
          <div className="side-edit-options">
            ...
            <div className="side-edit-color-picker">
              {PRESET_COLORS.map(c => (
                <button 
                  key={c} 
                  className={`side-edit-color-dot ${editState.color === c ? 'active' : ''}`}
  ...
  return (
    <li 
      className={`side-todo-item ${item.completed ? 'completed' : ''} ${isSelected ? 'selected' : ''}`}
      style={{ 
    ...
    >
      <div className="side-todo-item-header">
        <button 
          className="side-todo-toggle-btn"
          onClick={(e) => {
            e.stopPropagation();
